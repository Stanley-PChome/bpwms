...
namespace Wms3pl.Datas.F00
{
  ...
  {
    public IQueryable<F009001> GetData(string name)
    {
      var param = new object[] { name };
      var sql = @"SELECT * FROM F009001 WHERE NAME = @p0 AND STATUS IS NULL";
      return SqlQuery<F009001>(sql, param);
    }
    ...
    public void InsertLog(string dcCode, string gupCode, string custCode, string apiName, string sendData, string returnData, string errMsg, string status, DateTime startTime)
    {
      ...
      var sql = @" INSERT INTO F009001(DC_CODE,GUP_CODE,CUST_CODE,NAME,SEND_DATA,RETURN_DATA,ERRMSG,STATUS,
