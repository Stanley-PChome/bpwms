		...
		public IQueryable<CustWms_F050101_Detail> GetCustWmsDetails(string dcCode, string gupCode, string custCode, List<string> custOrdNos)
		{
			...
			var sql = @"
				SELECT 
					A.ORD_NO,
					A.CUST_ORD_NO,
					CASE WHEN E.PROC_FLAG IS NULL THEN '-1' ELSE E.PROC_FLAG END AS PROC_FLAG,
					A.DC_CODE,
					A.GUP_CODE,
					A.CUST_CODE,
					E.ORD_NO AS F050301_ORD_NO,
					E.STATUS AS F050801_STATUS
				FROM F050101 A
				LEFT JOIN 
				(SELECT B.DC_CODE,B.GUP_CODE,B.CUST_CODE,B.ORD_NO,B.PROC_FLAG,D.STATUS
				 FROM F050301 B 
				 JOIN F05030101 C ON C.DC_CODE = B.DC_CODE AND C.GUP_CODE = B.GUP_CODE AND C.CUST_CODE = B.CUST_CODE AND C.ORD_NO = B.ORD_NO
				 JOIN F050801 D ON C.DC_CODE = D.DC_CODE AND C.GUP_CODE = D.GUP_CODE AND C.CUST_CODE = D.CUST_CODE AND C.WMS_ORD_NO = D.WMS_ORD_NO
				) E 
				ON A.DC_CODE = E.DC_CODE AND A.GUP_CODE = E.GUP_CODE AND A.CUST_CODE = E.CUST_CODE AND A.ORD_NO = E.ORD_NO
				WHERE A.STATUS <> '9'
				AND A.DC_CODE = @p0
				AND A.GUP_CODE = @p1
				AND A.CUST_CODE = @p2
