...
{
	public partial class F060404Repository
	{
		public void UpdateF060404(List<string> docIds)
		{
		...
		public IQueryable<TaskDispatchData> GetF060404Datas(string dcCode, string gupCode, string custCode, DateTime? beginCreateDate, DateTime? endCreateDate, string status, List<string> docNums, List<string> taskNums)
		{
			var sql = @"SELECT TOP 100
						...
						A.CHECK_CODE,
						(SELECT TOP 1 NAME FROM F000904 WHERE TOPIC = 'F060404' AND SUBTOPIC = 'CMD_TYPE' AND VALUE = A.CMD_TYPE) CMD_TYPE_NAME,
						A.STATUS,
						...
						A.CRT_DATE ,
						A.UPD_DATE 
						FROM F060404 A WHERE A.DC_CODE = @p0
						AND A.GUP_CODE = @p1
						AND A.CUST_CODE = @p2";
		...
		public IQueryable<F060404> GetMaxDocId(List<string> wmsNos)
		{

			if (wmsNos.Any() || wmsNos != null)
			{
				var param = new List<object>();
				var sql = @"SELECT * FROM F060404 WHERE DOC_ID IN (SELECT  MAX(A.DOC_ID) DOC_ID FROM (SELECT B.WMS_NO,B.DOC_ID  FROM F060404 B WHERE 1=1";
				sql += param.CombineSqlInParameters(" AND B.WMS_NO", wmsNos);
				sql += @" ) A GROUP BY A.WMS_NO)";
				return SqlQuery<F060404>(sql, param.ToArray());
			}
			else
			{
				return new List<F060404>().AsQueryable();
			}
		...
		public IQueryable<string> GetWmsNoByWmsNo(List<string> docIds)
		{
			...
			{
				var param = new List<object>();
				var sql = @"SELECT WMS_NO FROM F060404 WHERE 1=1";
				sql += param.CombineSqlInParameters(" AND DOC_ID", docIds);
