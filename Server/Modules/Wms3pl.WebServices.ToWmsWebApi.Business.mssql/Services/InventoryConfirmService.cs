...
namespace Wms3pl.WebServices.ToWmsWebApi.Business.mssql.Services
{
    public class InventoryConfirmService : BaseService
    {
        ...
        private ApiResult ImportInventoryConfirmResults(string dcCode, string gupCode, string custCode)
        {
            ...
            #region 主要邏輯
            // 取得要執行盤點完成結果回傳資料
            var f060402s = f060402Repo.GetDatasForExecute(dcCode, gupCode, custCode);
                ...
                executeDatas.ForEach(f060402 =>
                {
                    ...
                    else
                    {
                        ...
                        if (f140101.ISSECOND == "0")
                        {
                            ...
                            // 取得實際庫存數
                            var stockDatas = f1913Repo.GetStockQtyByInventory(dcCode, gupCode, custCode, f060402.WAREHOUSE_ID,
                                currF140104s.Select(x => new StockDataByInventoryParam
                                {
                                    ITEM_CODE = x.F140104.ITEM_CODE,
                                    VALID_DATE = x.F140104.VALID_DATE,
                                    MAKE_NO = x.F140104.MAKE_NO,
                                    LOC_CODE = x.F140104.LOC_CODE,
                                    ENTER_DATE = x.F140104.ENTER_DATE,
                                    BOX_CTRL_NO = x.F140104.BOX_CTRL_NO,
                                    PALLET_CTRL_NO = x.F140104.PALLET_CTRL_NO
                                }).ToList());
                        ...
                        else
                        {
                            ...
                            // 取得實際庫存數
                            var stockDatas = f1913Repo.GetStockQtyByInventory(dcCode, gupCode, custCode, f060402.WAREHOUSE_ID,
                                currF140105s.Select(x => new StockDataByInventoryParam
                                {
                                    ITEM_CODE = x.F140105.ITEM_CODE,
                                    VALID_DATE = x.F140105.VALID_DATE,
                                    MAKE_NO = x.F140105.MAKE_NO,
                                    LOC_CODE = x.F140105.LOC_CODE,
                                    ENTER_DATE = x.F140105.ENTER_DATE,
                                    BOX_CTRL_NO = x.F140105.BOX_CTRL_NO,
                                    PALLET_CTRL_NO = x.F140105.PALLET_CTRL_NO
                                }).ToList());
        ...
        private IQueryable<StockDataByInventory> GetStockData(IQueryable<StockDataByInventory> data, string itemCode, string makeNo, DateTime validDate, string locCode, DateTime enterDate, string boxCtrlNo, string palletCtrlNo)
        {
