...
namespace Wms3pl.WebServices.ToWmsWebApi.Business.mssql.Services
{
		...
		private ApiResult ImportOutboundConfirmResults(string dcCode, string gupCode, string custCode)
    {
      ...
      #region 主要邏輯
      // 取得要執行出庫完成結果回傳資料
      var f060202s = f060202Repo.GetDatasForExecute(dcCode, gupCode, custCode);

      // 取得原任務單號
      var docIds = f060202s.Select(x => x.DOC_ID).ToList();

      // 取得出庫任務完成結果回傳明細資料
      var f060203s = f060203Repo.GetDatasForExecute(docIds);

      // 取得出貨容器紀錄
      var f060205s = f060205Repo.GetDatasForDocIds(docIds);

      // 取得出貨明細容器紀錄資料檔資料
      var f060206s = f060206Repo.GetDatasForDocIds(docIds);
              ...
              ApiLogHelper.CreateApiLogInfo(dcCode, gupCode, custCode, "ImportOutboundConfirmResults_Record", new { F060202 = f060202, F060203 = currF060203 }, () =>
              {
                if (f060202.WMS_NO.StartsWith("T")) // 調撥單
                {
                  ...
                  sharedService.AllocationConfirm(new AllocationConfirmParam
                  {
                    ...
                    Details = currF060203.Select(x => new AllocationConfirmDetail
                    {
                      Seq = Convert.ToInt16(x.ROWNUM),
                      Qty = x.SKUQTY
                    }).Distinct().ToList()
                  });
                }
                else if (f060202.WMS_NO.StartsWith("O") || f060202.WMS_NO.StartsWith("P")) // 出貨單、揀貨單
                {
                  ...
                  sharedService.PickConfirm(new PickConfirmParam
                  {
                    ...
                    Details = currF060203.Select(x => new PickConfirmDetail
                    {
                      Seq = x.ROWNUM.ToString().PadLeft(4, '0'),
                      Qty = x.SKUQTY
                    }).Distinct().ToList()
                  });
                }
                ...
                {
                  containerService.CreateContainer(currF060206.Select(x => new CreateContainerParam
                  {
                    DC_CODE = dcCode,
                    GUP_CODE = gupCode,
                    CUST_CODE = custCode,
                    CONTAINER_CODE = x.CONTAINERCODE,
                    CONTAINER_TYPE = "0",
