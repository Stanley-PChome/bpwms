...
{
	/// <summary>
	/// Custom DocumentPaginator Model(DataGrid Print)
	/// </summary>
	public class CustomDataGridDocumentPaginator : DocumentPaginator
	{
		...
		/// <summary>
		/// Custom column Proportion(define Bisects)
		/// </summary>
		public double[] CustomizeProportion { get; set; }
		...
		public override DocumentPage GetPage(int pageNumber)
		{
			...
			var viewSource = CollectionViewSource.GetDefaultView(_documentSource.ItemsSource);
			if (viewSource != null)
			{
			...
			if (tableGrid != null)
			{
				...
				for (int index = startPos; index < endPos && index < itemsSource.Count; index++)
				{
					...
					if (rowIndex > 0)
					{
						...
						{
							var alernatingRowBorder = new Border{};
				...
				foreach (DataGridColumn column in _documentSource.Columns)
				{
					if (column.Visibility == Visibility.Visible)
					{
						AddTableColumn(table,
							CustomizeProportion == null || !CustomizeProportion.Any() ? 1 : (CustomizeProportion.Count() > columnIndex ? CustomizeProportion[columnIndex] : 1),
							columnIndex,
							column);
						columnIndex++;
					}
		...
		private void AddTableColumn(Grid grid, double setColumn, int columnIndex, DataGridColumn column)
		{
			double proportion = column.Width.Value / (this.PageSize.Width - (this.PageMargin.Left + this.PageMargin.Right));

			var colDefinition = new ColumnDefinition { };
			colDefinition.Width = new GridLength(setColumn, GridUnitType.Star);
