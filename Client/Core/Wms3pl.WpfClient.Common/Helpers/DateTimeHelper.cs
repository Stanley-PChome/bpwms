...
namespace Wms3pl.WpfClient.Common.Helpers
{
	public static class DateTimeHelper
	{
		...
		/// <param name="formats">轉換的日期格式</param>
		public static DateTime ConversionDate(string date, string formats)
		{
			return ConversionDate(date, new List<string>() { formats });
		}
		/// <summary>
		/// 轉換字串為指定日期格式
		/// </summary>
		/// <param name="date">欲轉換的日期字串</param>
		/// <param name="formats">轉換的日期格式(若可能有多種輸入情形)</param>
		public static DateTime ConversionDate(string date, List<string> formats)
		{
			DateTime result;
			TryConversionDate(date, formats, out result);
			return result;
		}
		/// <summary>
		/// 轉換字串為指定日期格式
		...
		/// <returns></returns>
		public static bool TryConversionDate(string date, List<string> formats, out DateTime result)
		{
