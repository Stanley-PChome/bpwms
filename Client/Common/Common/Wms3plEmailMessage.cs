...
{
	public class Wms3plEmailMessage
	{
		...
		public Wms3plEmailMessage(EmailTraceListenerData configurationData, LogEntry logEntry, ILogFormatter formatter)
		{
		...
		/// <param name="formatter">The Formatter <see cref="ILogFormatter"/> which determines how the 
		/// email message should be formatted</param>
		public Wms3plEmailMessage(string toAddress, string fromAddress, string subjectLineStarter, string subjectLineEnder, string smtpServer, int smtpPort, LogEntry logEntry, ILogFormatter formatter)
		{
		...
		/// <param name="formatter">The Formatter <see cref="ILogFormatter"/> which determines how the 
		/// email message should be formatted</param>
		public Wms3plEmailMessage(string toAddress, string fromAddress, string subjectLineStarter, string subjectLineEnder, string smtpServer, int smtpPort, string message, ILogFormatter formatter)
		{
			this.configurationData = new EmailTraceListenerData(toAddress, fromAddress, subjectLineStarter, subjectLineEnder, smtpServer, smtpPort, string.Empty);
		...
		protected MailMessage CreateMailMessage()
		{
            ...
            var custCode = logEntry.ExtendedProperties["CUST_CODE"] == null ? "??????" : logEntry.ExtendedProperties["CUST_CODE"].ToString();
            var custName = logEntry.ExtendedProperties["CUST_NAME"] == null ? "不知名客戶" : logEntry.ExtendedProperties["CUST_NAME"].ToString();

            string sendToSmtpSubject = string.Format("[{0}][{1}]{2}",
                custCode, custName, header + logEntry.Severity.ToString() + footer);
		...
		{
			var smtpClient = new SmtpClient(configurationData.SmtpServer, configurationData.SmtpPort);
