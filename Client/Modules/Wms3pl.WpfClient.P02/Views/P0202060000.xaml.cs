...
{
	public partial class P0202060000 : Wms3plUserControl
	{
		public P0202060000()
		{
			InitializeComponent();
			...
			Vm.ShowP0202060100 += ShowP0202060100;
			Vm.OnDcCodeChanged += ShowDeviceWindowAndSetF910501;
			Vm.SetPurchaseNoFocus += SetPurchaseNoFocus;
		}

		private void ShowContainerDetail()
		{
			var win = new P0202060200(Vm.SelectedDc);
			win.ShowDialog();
		}


		private void ShowP0202060100()
		{
			var win = new P0202060100(Vm.SelectedData.DC_CODE, Vm.SelectedData.GUP_CODE, Vm.SelectedData.CUST_CODE, Vm.SelectedData.REAL_RT_NO, Vm.SelectedData.REAL_RT_SEQ);
			var res = win.ShowDialog();
			if(!res.HasValue || res.Value == false)
				Vm.SearchCommand.Execute(null);
			else
			{
				if(Vm.DgList.Where(x=> x.RT_SEQ != Vm.SelectedData.REAL_RT_SEQ && x.STATUS == "3").Any())
					Vm.SearchCommand.Execute(null);
				else
				{
					Vm.Init();
		...
		private void ContainerDetail_OnClick(object sender, RoutedEventArgs e)
		{
			var win = new P0202060200(Vm.SelectedDc);
			win.ShowDialog();
		}
