...
namespace Wms3pl.ScheduleModule.Consoles.HCTGetReply
{
	class Program
	{
		...
		private static string _scheduleName = "HCTGetReply";
		...
		private static void GetHctCustomerSetting()
		{
		...
		private static bool GetHCTStatusList()
		{
			var proxy = new wcf.SharedWcfServiceClient();
			#region 取得狀態項目資料
			try
		...
		private static void ExecGetReplyData()
		{
			...
			{
				ftpFiles = ftpFiles.Where(x => x.ToUpper().Contains(".TXT") && x.ToUpper().Contains(string.Format("ZS_NEW_{0}_{1}", _argConfig.CustomerId.ToUpper(),DateTime.Today.ToString("yyyyMMdd")))).ToList();
			...
			else
			{
				...
				{
					UpdateDBLogIsSuccess(id, "1", "FTP上無TXT檔案!!");
					Log("FTP上無TXT檔案!!");
				}
			}
			#endregion
		...
		{
			var id = InsertDBLog(_argConfig.DcCode, _argConfig.GupCode, _argConfig.CustCode, _scheduleName);
			var files = Directory.GetFiles(_appConfig.FilePath).Select(x => Path.GetFileName(x)).Where(x => x.ToUpper().Contains(".TXT") && x.ToUpper().Contains(string.Format("ZS_NEW_{0}_{1}", _argConfig.CustomerId.ToUpper(), DateTime.Today.ToString("yyyyMMdd")))).ToList();
			var bkfiles = Directory.GetFiles(_appConfig.LocalBackupPath).Select(x => Path.GetFileName(x)).Where(x => x.ToUpper().Contains(".TXT") && x.ToUpper().Contains(string.Format("ZS_NEW_{0}_{1}", _argConfig.CustomerId.ToUpper(), DateTime.Today.ToString("yyyyMMdd")))).ToList();
			...
			{
				UpdateDBLogIsSuccess(id, "1", string.Format("路徑：{0} 無TXT檔案!!", _appConfig.FilePath));
				Log(string.Format("路徑：{0} 無TXT檔案!!", _appConfig.FilePath));
			}
		}
		...
		private static void UpdateF050901Data(string fileName, int id)
		{
			...
			var fileData = new List<HCTGetReply>();
			#region 取得檔案內容
			string line;
			...
			while ((line = file.ReadLine()) != null)
			{
				...
				string[] str = GetStringList(line).ToArray();
				var data = new HCTGetReply();
				data.CONSIGN_NO = str[0];
				data.CUST_ID = str[4];
				data.STATUS_ID = str[20];
				data.IN_DATE = str[17] + str[23];
				fileData.Add(data);
			}
			file.Close();
		...
		private static List<string> GetStringList(string line)
		{
			int i = 0;
			List<string> strList = new List<string>();
			strList.Add(line.Substring(i, 10));
			i += 10;
			strList.Add(line.Substring(i, 1));
			i += 1;
			strList.Add(line.Substring(i, 4));
			i += 4;
			strList.Add(line.Substring(i, 3));
			i += 3;
			strList.Add(line.Substring(i, 11));
			i += 11;
			strList.Add(line.Substring(i, 6));
			i += 6;
			strList.Add(line.Substring(i, 20));
			i += 20;
			strList.Add(line.Substring(i, 8));
			i += 8;
			strList.Add(line.Substring(i, 6));
			i += 6;
			strList.Add(line.Substring(i, 8));
			...
			strList.Add(line.Substring(i, 8));
			i += 8;
			strList.Add(line.Substring(i, 5));
			i += 5;
			strList.Add(line.Substring(i, 7));
			i += 7;
			strList.Add(line.Substring(i, 2));
			i += 2;
			strList.Add(line.Substring(i, 1));
			i += 1;
			strList.Add(line.Substring(i, 5));
