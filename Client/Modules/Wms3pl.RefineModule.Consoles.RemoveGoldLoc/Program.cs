...
namespace Wms3pl.RefineModule.Consoles.RemoveGoldLoc
{
	class Program
	{
		...
		private static void RemoveGoldLocs()
		{
			...
			ConsoleHelper.Log($" *** RemoveGoldLocs排程Start ***");
			var wcf = new wcfR01.R01WcfServiceClient();
			var result = WcfServiceHelper.ExecuteForConsoleBySchemaName(wcf.InnerChannel
															, () => wcf.GetSchOrderData(7, 14), false, _appConfig.SchemaName, "RemoveGoldLocs_GetSchOrderData");
			...
			foreach (var item in groupData)
			{
				...
				if (resultImportExcel)
				{
					...
					var resultP70 = WcfServiceHelper.ExecuteForConsoleBySchemaName(wcf.InnerChannel, () => wcf.InsertF700501(wcf700501), false, _appConfig.SchemaName, "RemoveGoldLocs_InsertF700501");

				}
			}
			ConsoleHelper.Log($"*** RemoveGoldLocs排程 End ***");
		...
		private static bool ImportExcel(List<wcfR01.SchF700501Data> importData, string fileName,out string filePath)
		{
			...
			using (SaveFileDialog saveFileDialog = new SaveFileDialog())
			{
				...
				excelExportService.CreateNewSheet("移除黃金揀貨區名單");
