...
{
    public partial class P1920060000_ViewModel : InputViewModelBase
    {
        ...
        private string _COUDIVID = string.Empty;
        //[Required(ErrorMessage = Resources.Resources.Required_ErrorMessage, AllowEmptyStrings = false)]
        public string COUDIVID
        {
            get { return _COUDIVID; }
            set
            {
                _COUDIVID = value;
                RaisePropertyChanged("COUDIVID");
            }
        }
        ...
        private List<F1933> _DataList;
        public List<F1933> DataList
        {
            get
            {
                if (_DataList == null) _DataList = new List<F1933>();
                return _DataList;
            }
            set
            {
        ...
        private F1933 _selectedData;

        public F1933 SelectedData
        {
        ...
        private void DoSearch()
        {
            ...
            this.DataList = (proxy.CreateQuery<F1933>("GetF1933sByCOUDIVID")
                 .AddQueryOption("COUDIV_ID", "'" + this._COUDIVID + "'")
              ).ToList();
        ...
        private void DoDelete()
        {
            ...
            var pier = (from a in _proxy.F1933s
                        where a.COUDIV_ID.Equals(SelectedData.COUDIV_ID)
                        select a).FirstOrDefault();
            if (pier == null)
            {
                DialogService.ShowMessage(Properties.Resources.P1920060000_NoCOUDIV_ID);
                return;
            }
            else
            {
        ...
        private void DoSave()
        {
            ...
            if (string.IsNullOrEmpty(SelectedData.COUDIV_ID) || string.IsNullOrWhiteSpace(SelectedData.COUDIV_ID))
            {
                isValid = false;
                DialogService.ShowMessage(Properties.Resources.P1920060000_InputCOUDIV_ID);
                return;
            }
            ...
            {
                var pier = _proxy.F1933s.Where(x => x.COUDIV_ID.ToLower().Equals(SelectedData.COUDIV_ID.ToLower())).AsQueryable().ToList().Count();
                if (pier != 0)
                {
                    isValid = false;
                    DialogService.ShowMessage(Properties.Resources.P1920060000_COUDIV_ID_Duplicate);
                    return;
                }
            }

            if (string.IsNullOrEmpty(SelectedData.COUDIV_NAME) || string.IsNullOrWhiteSpace(SelectedData.COUDIV_NAME))
            {
                isValid = false;
                DialogService.ShowMessage(Properties.Resources.P1920060000_InputCOUDIV_Name);
                return;
            }
        ...
        private void DoSaveAdd()
        {
            ...
            F1933.COUDIV_ID = SelectedData.COUDIV_ID;
            F1933.COUDIV_NAME = SelectedData.COUDIV_NAME.Trim();

            _proxy.AddToF1933s(F1933);
            _proxy.SaveChanges();
        ...
        {
            var F1933s = _proxy.F1933s.Where(x => x.COUDIV_ID == SelectedData.COUDIV_ID).AsQueryable().ToList();
            var F1933 = F1933s.FirstOrDefault();

            if (F1933 != null)
            {
                F1933.COUDIV_NAME = SelectedData.COUDIV_NAME;
                F1933.UPD_STAFF = _userId;
                F1933.UPD_NAME = _userName;
                F1933.UPD_DATE = DateTime.Now;
                _proxy.UpdateObject(F1933);
                _proxy.SaveChanges();
                ShowMessage(Messages.Success);
            }
            else
