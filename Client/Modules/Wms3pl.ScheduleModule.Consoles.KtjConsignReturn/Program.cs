...
namespace Wms3pl.ScheduleModule.Consoles.KtjConsignReturn
{
	class Program
	{
		...
		private static string _scheduleName = "KTJConsignReturn";
		...
		private static void ExecReturnData()
		{
			...
			wcf.KTJShipReturn[] resultData;
			try
			{
				resultData = WcfServiceHelper.ExecuteForConsoleBySchemaName(proxy.InnerChannel
																									 , () => proxy.GetKTJShipReturns(_argConfig), false, _appConfig.SchemaName);
			}
			catch (Exception ex)
			{
				Log(ex.Message);
			...
			var fileName = string.Format("SO{0}.csv", DateTime.Now.ToString("yyyyMMddHHmmss"));
			var fileFullName = Path.Combine(_appConfig.FilePath, fileName);
			var importResult = ImportCSV(resultData.ToList(), fileFullName);
			if (!importResult)
			{
				UpdateDBLogIsSuccess(id, "0", "產生csv檔失敗");
				Log("產生csv檔失敗");
				return;
			}
		...
		#region 產生CSV
		private static bool ImportCSV(List<wcf.KTJShipReturn> datas, string fileFullName)
		{
			try
			{
				using (var sw = new StreamWriter(fileFullName, false, Encoding.UTF8))
				{
					var sb = new StringBuilder();
					foreach (var item in datas)
					{
						//出貨日期|客戶編號|託運單號|寄貨人名稱|	
						sb.Append(string.Format("{0}|{1}|{2}|{3}|", item.SEND_DATE?.Trim(), item.CONTRACT_CUST_NO?.Trim(), item.CONSIGN_NO?.Trim(), item.SUPPLIER_NAME?.Trim()));
						//寄件人郵區|寄件人地址|寄件人電話|收件人姓名|收件人郵遞區號|
						sb.Append(string.Format("{0}|{1}|{2}|{3}|{4}|", item.SUPPLIER_ZIP?.Trim(), item.SUPPLIER_ADDRESS?.Trim(), item.SUPPLIER_PHONE?.Trim(), item.RECEIVER_NAME, item.RECEIVER_ZIP_CODE?.Trim()));
						//收件人地址|收件人電話|件數|才數|代收金額|
						sb.Append(string.Format("{0}|{1}|{2}|{3}|{4}|", item.RECEIVER_ADDRESS?.Trim(), item.RECEIVER_PHONE?.Trim(), item.PIECES?.Trim(), item.VOLUME_QTY?.Trim(), item.COLLECT_AMT?.ToString().Trim()));
						//備註|配送時段|指送日期|出貨單號|
						sb.Append(string.Format("{0}|{1}|{2}|{3}", item.MEMO?.Trim(), item.ASSIGN_TIME?.Trim(), item.A_ARRIVAL_DATE?.ToString("yyyyMMdd").Trim(), item.CUST_ORD_NO?.Trim()));
		...
		public static bool SendMail(string filePath, SendMailType sendType, string body = null)
		{
			try
			{
				...
				if (SendMailType.FtpUpload == sendType)
				{
					...
					mailSubject = string.Format(_appConfig.UploadSubject, DateTime.Today.ToString("yyyy/MM/dd"), "KTJ出貨託運檔", "已上傳");
				}
		...
		private static void GetKtjCustmerSetting()
		{
			var proxy = new wcf.SharedWcfServiceClient();
		...
		#region 更新託運單與派車狀態
		private static bool UpdateStatus(List<wcf.KTJShipReturn> datas)
		{
			var proxy = new wcf.SharedWcfServiceClient();
			var itemData = ExDataMapper.MapCollection<wcf.KTJShipReturn, wcf.KTJShipReturn>(datas).ToArray();
			var updateResult = WcfServiceHelper.ExecuteForConsoleBySchemaName(proxy.InnerChannel
																						, () => proxy.UpdateStatusForKTJ(itemData), false, _appConfig.SchemaName);
