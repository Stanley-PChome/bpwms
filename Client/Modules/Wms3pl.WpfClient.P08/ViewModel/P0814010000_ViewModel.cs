...
namespace Wms3pl.WpfClient.P08.ViewModel
{
    ...
    public virtual void DoEndPackingComplete()
    {
      ...
      else// 是否過刷=是
      {
        ...
        private CloseShipBoxRes DoCloseBox(int? packageBoxNo = null, string itemCode = null, bool? isIsAppendBox = false)
        {
    ...
    protected void DoSearchContainer()
    {
			...
			if (!chkRes.IsSpecialOrder)//8.如果[E]. Result.IsSuccessed = true AND[E]. IsSpecialOrder=0 (非特殊結構訂單)
      {
        ...
        if (!WmsOrdData.Result.IsSuccessed)
        {
          ...
          else if (WmsOrdData.Status == 5 || WmsOrdData.Status == 9)
          {
      ...
      if (DetailData.Any())
        ...
        private bool ScanBarcode(int packQty, string action)
        {
            ...
            if (isCarton && CloseByBoxno != "1")
            {
                ...
                if (ShowConfirmMessage("您設定為不需要刷讀紙箱關箱，你是否要手動刷讀紙箱關箱？") != UILib.Services.DialogResponse.Yes)
                {
            ...
            if (scanRes.IsSuccessed)
            {
                ...
                else
                {
                    ...
                    _closeBoxParam = new CloseBoxParam { PackageBoxNo = scanRes.PackageBoxNo, ItemCode = scanRes.ItemCode };
