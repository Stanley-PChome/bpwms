...
namespace Wms3pl.RefineModule.Consoles.ItemTurnoverRate
{
	class Program
	{
		...
		private static void ItemTurnoverRates()
		{
			...
			ConsoleHelper.Log($" *** ItemTurnoverRates排程Start ***");
			var wcf = new wcfR01.R01WcfServiceClient();
			var result = WcfServiceHelper.ExecuteForConsoleBySchemaName(wcf.InnerChannel
															, () => wcf.GetSchOrderAllData(90), false,_appConfig.SchemaName, "ItemTurnoverRates");
			...
			foreach (var item in groupData)
			{
				...
				if (resultExcel)
				{
					...
					var resultP70 = WcfServiceHelper.ExecuteForConsoleBySchemaName(wcf.InnerChannel, () => wcf.InsertF700501(wcf700501), false, _appConfig.SchemaName, "ItemTurnoverRates");
				}

			}

			ConsoleHelper.Log($"*** ItemTurnoverRate排程 End ***");
