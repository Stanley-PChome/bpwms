...
{
    public partial class P1504010000_ViewModel : InputViewModelBase
	{
		...
		private void InitControls()
		{
			...
			dpDELV_DATE = DateTime.Today;
			STATUSList = GetBaseTableService.GetF000904List(FunctionCode, "F051206", "STATUS", true);
	
			if (STATUSList.Any())
				cbSTATUS = STATUSList.First().Value;

			RETURN_FLAGList = GetBaseTableService.GetF000904List(FunctionCode, "F051206", "RETURN_FLAG", false);
		...
		public ObservableCollection<NameValuePair<string>> GetPICK_TIMEList()
		{
			...
			.Where(x => x.DC_CODE.Equals(cbDC_CODE_ALLOT))
		...
		#region 批次日期
		private DateTime _dpDELV_DATE;
		public DateTime dpDELV_DATE
		{
			get { return _dpDELV_DATE; }
			set
			{
				_dpDELV_DATE = value;
				...
				RaisePropertyChanged("dpDELV_DATE");
			}
		}
		#endregion
		...
		private void DoEditCompleted()
		{
			showSaveBtn = true;
			EditAction();
			var list = GetBaseTableService.GetF000904List(FunctionCode, "F051206", "RETURN_FLAG", false);
			RETURN_FLAGList = list.Where(x => x.Value != "4" && x.Value!="5").ToList();
            cbRETURN_FLAG = RETURN_FLAGList.Select(x => x.Value).FirstOrDefault();
        }
		#endregion Edit
		...
		private void DoSaveAdd()
		{
			IsSave = true;
				...
				{
					IsSave = false;
					DialogService.ShowMessage(Properties.Resources.P1504010000_ViewModel_CheckItemDetail);
					return;
				}
				if (selectedLackList_allot.Any(x => x.LACK_QTY <= 0))
				{
					IsSave = false;
					DialogService.ShowMessage(Properties.Resources.P1504010000_ViewModel_OutofStockQtyGreaterThanZero);
					return;
				}
				if (selectedLackList_allot.Any(o => o.LACK_QTY > o.SRC_QTY))
				{
					IsSave = false;
					DialogService.ShowMessage(Properties.Resources.P1504010000_ViewModel_OutofStockQtyNotOverTransQty);
					return;
				}
				if (PickSelected2_Allot.STATUS == "0")
				{
					IsSave = false;
					DialogService.ShowMessage(Properties.Resources.P1504010000_ViewModel_CheckAllocNoStatus);
					return;
				}
		...
		private void DoSaveEdit()
		{
            ...
            var selectedLackList_allot = LackList_Allot.Where(x => x.ISSELECTED).ToArray();
            if (selectedLackList_allot.Count() == 0)
            {
                IsSave = false;
                DialogService.ShowMessage(Properties.Resources.P1504010000_ViewModel_CheckItemDetail);
            ...
            //調撥 若已找到商品 不用檢核數量 0
            if (selectedLackList_allot.Any(x => x.LACK_QTY == 0 && x.STATUS == "3"))
			{
				IsSave = false;
				DialogService.ShowMessage(Properties.Resources.P1504010000_ViewModel_OutofStockQtyNotZero);
				return;
			}
			if (selectedLackList_allot.Any(o => o.LACK_QTY < 0))
			{
				IsSave = false;
				DialogService.ShowMessage(Properties.Resources.P1504010000_ViewModel_OutofStockQtyGreaterThanZero);
				return;
			...
			var wcfF051206LackList = ExDataMapper.MapCollection<F051206LackList_Allot, wcf.F051206LackList_Allot>(selectedLackList_allot).ToArray();
			var proxy = new wcf.P06WcfServiceClient();
		...
		private void DoSaveCompleted()
		{
			if (IsSave)
			{
				...
				SHOWADDPAGE = "Collapsed";
				showSaveBtn = false;
				RETURN_FLAGList = GetBaseTableService.GetF000904List(FunctionCode, "F051206", "RETURN_FLAG", true);
